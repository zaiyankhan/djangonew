...
class RedirectAuthenticatedUserMixin(object):
    def dispatch(self, request, *args, **kwargs):
        ...
        # (end WORKAROUND)
        if is_authenticated(request.user) and not request.user.is_superuser and app_settings.AUTHENTICATED_LOGIN_REDIRECTS:
...
class LoginView(RedirectAuthenticatedUserMixin,
    ...
    def get_context_data(self, **kwargs):
        if self.request.is_ajax():
            self.template_name = 'account/login_ajax.html'
        ...
        initial = {}
        try:
            if self.request.geo_data['country_code']:
                initial.update({
                    'country': self.request.geo_data['country_code']
                })
        except Exception:
            pass
        try:
            if self.request.geo_data['city']:
                initial.update({
                    'city': self.request.geo_data['city']
                })
        except Exception:
            pass
        try:
            if self.request.geo_data['region']:
                initial.update({
                    'state': self.request.geo_data['region']
                })
        except Exception:
            pass
        signup_form = get_form_class(app_settings.FORMS, 'signup', self.signup_form)(initial=initial)
...
class SignupView(RedirectAuthenticatedUserMixin, CloseableSignupMixin,
    ...
    def form_valid(self, form):
        ...
        mail_managers('New user registration at deepex.com',
                      'A new user %s (%s), has registered on the web site\n\n%s\n\nBusiness type:%s\n\nInterested in: %s' %
                      (self.user.get_full_name(), self.user.email, self.user.email_address_details,
                       self.user.business_type, self.user.interested),
                      fail_silently=True)
        try:
    ...
    def get_context_data(self, **kwargs):
        ...
        redirect_field_value = get_request_param(self.request,
                                                 redirect_field_name)
        if self.request.is_ajax():
            self.template_name = 'account/login_ajax_signup.html'
        ret.update({"login_url": login_url,
                    'loginform': get_form_class(app_settings.FORMS, 'login', self.login_form),
                    "redirect_field_name": redirect_field_name,
                    "redirect_field_value": redirect_field_value})
        return ret
